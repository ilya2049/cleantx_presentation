...
```go file=./src/what_is_tx.go title="Что такое транзакция?"
```
...
## Абстрактные транзакции

- Наиболее актуальны в чистой архитектуре
- Применяются в слое приложения
- Инкапсулируют драйвер базы данных
...
```txt file=./src/base/app_structure.txt title="Структура проекта"
```

```txt 16:17 file=./src/base/app_structure.txt title="Структура проекта" subtitle="Модели предметной области"
```

```txt 12:13 file=./src/base/app_structure.txt title="Структура проекта" subtitle="Исполнитель команд"
```

```txt 18:20 file=./src/base/app_structure.txt title="Структура проекта" subtitle="Реализация репозитория"
```

```txt 14:15 file=./src/base/app_structure.txt title="Структура проекта" subtitle="Командный интерфейс"
```

```txt 2:6 file=./src/base/app_structure.txt title="Структура проекта" subtitle="Точки входа"
```

```txt file=./src/base/app_structure.txt title="Структура проекта"
```

```txt 11:20 file=./src/base/app_structure.txt title="Слои сервиса" subtitle="В файловой системе"
```

```txt file=./src/base/clean_architecture.txt title="Слои сервиса" subtitle="Концептуально"
```
...
```go 34:54 file=./src/base/app/command_executor.go title="Внедрение транзакции" subtitle="Потерянное обновление"
```

```go 35:38 file=./src/base/app/command_executor.go title="Внедрение транзакции" subtitle="Два конкурентных запроса считали, что 1 доктор дежурит"
```

```go 49:51 file=./src/base/app/command_executor.go title="Внедрение транзакции" subtitle="Два конкурентных запроса завершили смены двух докторов"
```
