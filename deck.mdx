...
Мы посмотрели на структуру проекта "с высоты птичьего полета". 
Теперь повнимательнее приглядимся к каждому архитектурному слою приложения.
...
Мы увидели, что модель доктора позволяет определить, может ли доктор покинуть смену, 
или он - последний, и придется еще подежурить.

На очереди слой приложения.
